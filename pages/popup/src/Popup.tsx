    ...
    const load = () => {
      chrome.storage.local.get(SEMRUSH_BACKLINKS_STORAGE_KEY).then(raw => {
        const data = raw[SEMRUSH_BACKLINKS_STORAGE_KEY] as
          | Record<string, { items: unknown[]; total: number }>
          | undefined;
        if (!data) {
      ...
      func: async (url: string, pages: number) => {
        ...
        for (let p = 1; p < pages; p++) {
          ...
          try {
            ...
            const data = await res.json();
            window.postMessage(
              { type: 'SEMRUSH_BACKLINKS_RESPONSE', requestUrl: pageUrl.toString(), payload: data },
              '*',
            );
          } catch {
            /* skip failed pages */
  ...
  return (
    <div className={cn('App', isLight ? 'bg-slate-50' : 'bg-gray-800')}>
      <header className={cn('App-header', isLight ? 'text-gray-900' : 'text-gray-100')}>
        ...
        {onSemrushSection && (
          <section className={cn('mt-4 rounded p-3 text-left text-sm', isLight ? 'bg-blue-50' : 'bg-gray-700')}>
            <p className="mb-2 font-semibold">Semrush 反向链接</p>
            <p className="mb-2 opacity-90">
              当前页数据会在加载时自动抓取。若点「抓取全部页」报错，请先刷新本页、等列表加载完成后再试。
            </p>
